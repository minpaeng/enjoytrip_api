...
<div>
	...
	<div class="row-md-4 ms-5 me-5">
		...
		<form action="${root}/attraction/makePlan" method = "post">
			...
			<div id="poll-answer-list" class="row mb-3">
				<div class="row mb-1 poll-answer-list-item">
					<div class="col-md-10">
						<table id="top-table" class="row select" style="display: none">
							<thead>
								<tr>
									<th>대표이미지</th>
									<th>관광지명</th>
							...
							<tbody id="trip-select"></tbody>
						</table>
						<p><textarea id = "memo" name = "memo" cols="87" rows="10"></textarea></p>						
					</div>
				</div>
			</div>
		...
		<div class="row" style="font-family: 'Black Han Sans', sans-serif">
			<table id="under-table" class="table table-striped" style="display: none">
				<thead>
					<tr>
						<th>대표이미지</th>
						<th>관광지명</th>
...
/*         let townUrl =
      ...
      function buttonClicked(contentId){
    	  ...
    	  let url = `${root}/attraction/selectByContentId?contentId=`+ contentId;
    	  
    	  fetch(url, { method: "GET" })
          .then((response) => response.json())
          .then((data) => selectList(data));
    	  ...
    	  document.querySelector("#top-table").setAttribute("style", "display: ;");
            ...
            let list = data;
            let attractionList = ``;
            attractionList += '<div id = contentId name = contentId value = '+ list.contentId + '>'
            attractionList += '<input type = hidden name = contentId value = ' + list.contentId + '>'
            attractionList += '<tr onclick="moveCenter(list.latitude, list.longitude);">'
          		attractionList += '<td><img src='+list.firstImage+' width=100px></td>'
          		attractionList += '<td>'+list.title+'</td>'
          		attractionList += '<td>'+list.addr1+' '+list.addr2+'</td>'
          		attractionList += '</tr>'
          		attractionList += '</div>';
            	
     	console.log(document.getElementById("trip-select"));
      	document.getElementById("trip-select").innerHTML += attractionList;
    }
